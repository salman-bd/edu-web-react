...
export default function Header() {  
    ...
    const handleSearch = (event) => {  
        ...
        if (searchQuery.trim()) {  // Trim whitespace  
            const googleSearchUrl = `https://www.google.com/search?q=${encodeURIComponent(searchQuery)}`;  
            window.open(googleSearchUrl, '_blank'); // Opens the search in a new tab  
        } else {  
            // Display an inline message instead of alert  
            alert("Please enter a search query.");  
        }  
    };  

    const handleLogout = () => {  
        // Clear authentication state and close the mobile menu on logout  
        useAuthStore.getState().logout();  // Assuming you have a logout method  
            ...
            <nav className="bg-white border-gray-800 px-3 lg:px-12 py-2">  
                ...
                <div className="hidden lg:flex lg:items-center lg:justify-between">  
                    <ul className="flex space-x-8">  
                        {['/', '/about', '/contact', '/registration', '/admitcard', '/Find_You'].map((path, index) => (  
                            <li key={index}>  
                                <NavLink  
                                    to={path}  
                                    className={({ isActive }) =>  
                        ...
                        <ul className="flex flex-col items-center font-medium gap-2 text-3xl">  
                            ...
                            {isAuthenticated ? (  
                                <li className='flex items-center'>   
                                    ...
                                    <Logout onLogout={() => setMobileMenuOpen(false)} />  
                                    <button  
                                        onClick={handleLogout}  
